...
";
    ...
    // Stage 14: Crossing Remedial Intro (口訣複習)
    state_index++;
    mStages[state_index].m_strStageName = "crossing_remedial_intro";
    mStages[state_index].m_strSystemMessage = R"(你是一台名叫凱比的交通安全宣導機器人，正在和小朋友複習安全過馬路四步驟。請遵守以下規則：
        1. 使用非常簡單、親切的中文回答，不能使用其他語言。
        2. 所有數字必須使用對應的繁體中文字。
        3. 用很短的句子點出口訣與重點。
        4. 請不要輸出任何表情符號或括號。
        )";
    mStages[state_index].m_strFirstSentence = "沒關係，我們一起加油。口訣是一停二看三聽四走，記住就會很安全。";
    mStages[state_index].m_secDurationLimit = 20s;

    // Stage 15: Crossing Remedial Quiz (口訣小測驗)
    state_index++;
    mStages[state_index].m_strStageName = "crossing_remedial_quiz";
    mStages[state_index].m_strSystemMessage = R"(你是一台名叫凱比的交通安全宣導機器人，正在檢核安全過馬路四步驟。請遵守以下規則：
        1. 使用非常簡單、親切的中文回答，不能使用其他語言。
        2. 所有數字必須使用對應的繁體中文字。
        3. 一次只問一個問題。
        4. 請不要輸出任何表情符號或括號。
        )";
    mStages[state_index].m_strFirstSentence = "請跟著我說一次，一停二看三聽四走。現在告訴我，第四步要注意什麼？";
    mStages[state_index].m_secDurationLimit = 30s;
